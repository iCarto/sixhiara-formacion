...
A lo largo de este taller vamos a aprender conceptos como qué es una máquina virtual, qué prestaciones nos ofrecen o como instalar una máquina virtual en un entorno Windows.
...
4. Si queremos contar con las _Guest Additions_, éstas se instalarán sobre cada una de las MMVV. Su instalación es diferente de la instalación propia de VB. Estas sirven para mejorar la comunicación entre el anfitrión y una MV concreta.
...
- Descarga una sola vez una imagen del Sistema Operativo que puedes replicar para cada uno de los proyectos que desees que tengan configuraciones diferentes.  
...
- Una herramienta de consola: permite automatizar procesos en lugar de crear máquinas virtuales y entornos de desarrollo manualmente. Este lenguaje de scripting permite compartir configuraciones de cada proyecto, sin almacenar las máquinas virtuales completas, ahorrando espacio en disco.   
...
A continuación, vamos a comenzar a utilizar Vagrant. Para ello, abrimos una consola. Podemos utilizar la consola por defecto de Windows (cmd) o instalar alguna otra como [Cygwin](http://www.cygwin.com/) o [cmder](http://cmder.net/).
...
2. Navegamos hasta el directorio donde vayamos a crear la máquina virtual y ejecutamos el siguiente comando (sin el símbolo del dólar).
     - Para navegar por los directorios utilizamos el comando `cd`  + `nombre-de-carpeta` o `ruta`. 
     - Para subir un nivel `../`.
     
   Una vez en el directorio donde queremos crear la máquina virtual tecleamos en la consola:
   ...
   Un ejemplo sencillo de Vagrantfile podría ser el siguiente:
     ...
     Vagrant.configure("2") do |config|
         ...
         config.vm.box_url = config.vm.box_url =  "https://app.vagrantup.com/debian/boxes/contrib-stretch64"
     ...
     ```   
   En este caso, estamos indicando que use una imagen de `Ubuntu 16.04` de VirtualBox llamada `xenial64` y la URL de donde puede descargarla. También se indica qué carpetas queremos que se sincronicen en local con la máquina virtual, en este caso tendrá todo el código de un proyecto en una carpeta llamada `project` en mi entorno local. De esta manera podremos utilizar por ejemplo un editor de código como _Sublime Text_ o cualquier otro con el que nos sintamos cómodos. Todo lo que modifiquemos se modificará automáticamente en la carpeta `/project` de la maquina virtual linux que hemos creado. Tambien indicamos que los puertos `6543` y `5432` de la maquina virtual (`guest`) se relacionen con los puertos `9000` y `9001` de mi local (`host`). De esa forma podríamos usar el navegador local para correr los proyectos que se ejecutan en la máquina virtual usando el mismo puerto.   
   Por último, con el comando `config.vm.provision` indicamos la ruta a un script en `bash` con los comandos que instalarán en el OS todas las dependencias y software que necesitamos para crear nuestro entorno.   
...
4. Lamentablemente, Windows no viene con un cliente de línea de comandos `SSH`. Esto impica que las funciones `SSH` integradas de Vagrant no funcionarán para los usuarios de Windows. `PuTTY` es un cliente `SSH` con licencia libre. Si necesitamos conectarnos a la máquina por `SSH` instalaremos `PuTTY`. Dentro introducimos la IP y el puerto, otorgamos un nombre a la conexión y salvamos la información. Ahora podremos conectarnos al nuevo servidor a través de `PuTTY`. El nombre de usuario y contraseña es `vagrant` (configurable).
   ...
   Si estamos en un contexto diferente de Windows, podemos entrar por `SSH` en la máquina virtual con el comando:
    ```shell
    $ vagrant ssh
    ```
...
5. Otros comandos útiles:
   Parar la máquina virtual tienes el comando
    ```shell
    $ vagrant halt
    ```
